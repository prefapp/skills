...
5. **Fire the research subagents.** For each `research` ticket you just created, spin up a `/research` subagent to resolve it in parallel, capturing its findings on a throwaway `research/<name>` branch with a context pointer from the ticket. These branches need not be pushed; keep them local unless the repository workflow explicitly requires sharing them. After the ticket is resolved, delete the branch once the findings are preserved in a durable repo location and the ticket's pointer targets it. If branch creation is unavailable, write the findings directly to that durable location instead.
