...
Once the prototype has answered its question, capture the answer, then follow [SKILL.md](SKILL.md): lift the validated logic into the real module and, with explicit commit approval, preserve the complete prototype on a linked throwaway branch; otherwise delete it. Being one self-contained file, it stays trivially re-runnable there.
