## Skill files: minimum necessary text

Write `SKILL.md` and the reference/playbook files a skill discloses (anywhere
in `skills/workflow/` or `skills/firestartr/`) with the minimum text an agent
needs to understand the concepts and carry out the tasks — no more. See
`skills/workflow/writing-for-agents/SKILL.md` for the pruning technique
(no-ops, sediment, single source of truth).

This does not apply to files written for a human reader — `README.md` (root
or nested), `CONTEXT.md`, `docs/adr/*`, `THIRD_PARTY_NOTICES.md` — which
should stay as clear and welcoming as they need to be.
